    ...
    <v-container>
        <v-card>
            <v-toolbar flat color="blue" class="white--text">
                ...
                <v-toolbar-title>
                    <h3 class="font-weight-light">Leads</h3>
                </v-toolbar-title>
            </v-toolbar>
            ...
            <v-card-text>
                ...
                <v-row dense>
                    ...
                    <v-col cols="12" md="4">
                        <v-select v-model="query.seguimiento" :items="seguimiento" outlined small dense label="Estado" hide-details></v-select>
                    </v-col>
                    ...
                    <v-col cols="12">
                        <v-data-table :headers="headers" :items="items" disable-pagination hide-default-footer item-key="key" class="elevation-1 mt-2">
                            ...
                            <template v-slot:item.action="{ item }">
                                ...
                                <router-link :to="`/guardar-cliente?id=${item.id}`" class="action-buttons">
...
<script>
    ...
    import {
        headers_mixin
    } from '../mixins/potenciales_headers.js'
    ...
    export default {
        ...
        methods: {
            ...
            search_potenciales(page, query) {
                potencial_service.search_potencial(page, query).then(res => {
            ...
            debounceQuery: debounce(function(query) {
                this.search_potenciales(1, query)
            }, 800),
        },
