<template id="">
    <v-container>
        ...
        <v-card>
            ...
            <v-card-text>
                ...
                <v-form ref="form" v-model="valid" lazy-validation>
                    ...
                    <v-row dense>
                        <v-col cols="12" md="3">
                            <v-select v-model="item.tipo_medio" :items="tipos_medios" label="Tipo fuente"></v-select>
                        </v-col>

                        <v-col cols="12" md="3">
                            <v-select v-model="item.medio" :items="medios" label="Fuente"></v-select>
                        </v-col>
                    ...
                    <v-row dense>
                        ...
                        <v-col cols="12" md="3">
                            <v-select v-model="item.ultimo_servicio.status" :items="servicio_status" outlined small dense label="Estado mudanza" color="green"></v-select>
                        </v-col>
                    </v-row>
                    ...
                    <v-row dense>
                        <v-col cols="12">
                            ...
                            <v-btn :loading="isloading" v-if="item.id" @click="openHistorial" class="text-white ml-4" color="orange">historial</v-btn>
                            <v-btn :loading="isloading" v-if="item.id" :to="`/crear-presupuesto?cliente_id=${item.id}`" class="text-white ml-4" color="purple">Presupuesto</v-btn>
                        </v-col>
                    </v-row>
        ...
        <dialog-historial></dialog-historial>

    </v-container>
</template>

<script>
    import dialogHistorial from './dialogHistorial.vue'
    import {
        user_service
    ...
    export default {
        ...
        data() {
            return {
                medios: [],
                comerciales: [],
                servicio_status: ['pendiente', 'completado'],
                item: {
                    id: null,
                    comercial_id: null,
                    nombre: '',
        ...
        watch: {
            'item.tipo_medio': {
                ...
                handler(n) {
                    this.item.medio = ''
                    this.medios = (String(n).toLowerCase() == 'interno') ? this.medios_internos : this.medios_externos
                }
            }
        },
        ...
        methods: {
            openHistorial() {
                let id = this.item.ultimo_servicio.id
                let historial = this.item.ultimo_servicio.order_historial
