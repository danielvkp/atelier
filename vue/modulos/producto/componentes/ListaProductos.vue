    ...
    <v-container>
        <v-card>
            <v-toolbar flat color="blue" class="white--text">
                ...
                <v-toolbar-title>
                    <h3 class="font-weight-light">producto</h3>
                </v-toolbar-title>
            </v-toolbar>
            ...
            <v-card-text>
                ...
                <v-row dense class="mt-0">
                    ...
                    <v-col cols="12" md="4">
                        <v-btn to="/guardar-producto" class="mt-1 white--text" color="blue">
                            nuevo
                        </v-btn>
                    </v-col>
                </v-row>
                    ...
                    <v-col cols="12">
                        <v-data-table :search="search" items-per-page="-1" :headers="headers" :items="items" disable-pagination hide-default-footer item-key="key" class="elevation-1 mt-2">
                            ...
                            <template v-slot:item.action="{ item }">
                                ...
                                <router-link :to="`/guardar-producto?id=${item.id}`" class="action-buttons">
        ...
        <v-confirm v-on:delete="delete_producto">
        </v-confirm>
    </v-container>
</template>
...
<script>
    ...
    export default {
        ...
        methods: {
            ...
            delete_producto() {
                producto_service.delete_producto(this.item.id).then(res => {
            ...
            get_productos() {
                producto_service.get_productos().then(res => {
                    this.items = res.data
                }, res => {
