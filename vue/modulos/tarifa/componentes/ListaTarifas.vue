        ...
        <v-card>
            ...
            <v-card-text>
                ...
                <v-row dense class="mt-0">
                    <v-col cols="12" md="4">
                        <v-btn to="/guardar-tarifa" class="mt-1 white--text" color="blue">
                            nuevo
                        </v-btn>
                    </v-col>
                </v-row>
                    ...
                    <v-col cols="12">
                        <v-data-table :headers="headers" :items="items" disable-pagination hide-default-footer item-key="key" class="elevation-1 mt-2">
                            ...
                            <template v-slot:item.action="{ item }">
                                ...
                                <router-link :to="`/guardar-tarifa?id=${item.id}`" class="action-buttons">
...
<script>
    ...
    export default {
        ...
        methods: {
            ...
            delete_tarifa() {
                tarifa_service.delete_tarifa(this.item.id).then(res => {
            ...
            get_tarifas() {
                tarifa_service.get_tarifas().then(res => {
                    this.items = res.data
                }, res => {
